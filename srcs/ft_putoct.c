...
/*   Updated: 2020/01/27 23:11:58 by dtai             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static int	find_size_oct(unsigned long long nbr)
{
	int	ret;
...
static void	input_oct(unsigned long long nbr, char *print_out, int size)
{
	while (--size >= 0)
	{
		print_out[size] = nbr % 8 + '0';
		nbr /= 8;
	}
}

static int	ft_putoct_i(unsigned long long nbr, t_info *info)
{
	int		ret;
	char	*print_out;

	ret = find_size_oct(nbr);
	ret = info->s_precision > ret ? info->s_precision : ret;
	print_out = (char *)malloc(sizeof(char) * (ret + 1));
	print_out[ret] = '\0';
	input_oct(nbr, print_out, ret);
